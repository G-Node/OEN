    ...
    for prefix in ppty_dict.keys():
        ...
        if type(ppty_dict[prefix]) is list:
            for ppty in ppty_dict[prefix]:
                ...
                #if prefix=="rdfs:subClassOf": print prefix, " ", ppty.lower()
                if prefix=="rdfs:subClassOf" and ppty.lower() in ["oen_0000001", "unclassified term"]:
                    ...
                    if temp in range(10000000):
                        ...
                        #print  '    \t<' + prefix + ' rdf:resource="&oen_term;' + oen_idz + '"/>'
...
def OENnewClass(OWL, oen_id, oen_label, oen_source):
    #Original function kept for exposing principles of generalised OENimportedClass()
    stub = '\n    <!-- ' + OWL.namespace + '/' + oen_id + ' -->\n'
    stub += '\n    <owl:Class rdf:about="&oen_term;' + oen_id + '">\n'
    stub += '    \t<rdfs:label xml:lang="en">' + oen_label + '</rdfs:label>\n'
    stub += '    \t<dc:source xml:lang="en">' + oen_source + '</dc:source>\n'
    stub += '    </owl:Class>\n\n\n'
    #print stub
    OWL.doc += stub
    return OWL
...
out = OWL("http://purl.org/incf/ontology/ExperimentalNeurophysiology/oen_term.owl", "oen_term.owl", "http://purl.org/incf/ontology/ExperimentalNeurophysiology/oen_term.owl/properties")
#out.start()
for n in range(len(names)):
    out.addClass(names[n])
    out.assertEquivalentTriple(names[n], "hasColor", colors[n], "some")
    out.assertSubClassTriple(names[n], "hasGender", gender[n], "some")
out.end()
'''